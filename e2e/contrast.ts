 ...
 *    `color-mix()`. The hero aside is a `color-mix()` wash here, and the three
 *    status pills are `rgba()` tints that stack on whichever surface they land
 *    on. axe files those under `incomplete` rather than measuring them, and it
 *    is precisely that stacking that this helper caught: `--warn-text` was
 *    measured at 5:1 on the warn pill sitting on a panel, but inside the
 *    trapdoor `<details>` the same pill sits a surface deeper and the composite
 *    lightens — 4.48:1.
 *  - text faded by an ancestor's `opacity` — axe reads the declared `color`,
 *    which is not the colour that lands on screen. The hero subtitle renders at
 *    `opacity: .85` and disabled buttons at `.5`. Note this lab collapses its
 *    animations to 0.01ms under reduced motion, which preserves end states.
 *
 ...
 *    boxes are the `.mono-box` value blocks — `overflow-x: auto` around a
 *    `white-space: nowrap` 2048-bit integer, so most of every N, x, y, ℓ and π
 *    is outside its scroller's client box at any moment. Content scrolled past
 *    that box is not dimmed or partly drawn — it is absent from the frame, and
 ...
 *    fall through to white, inventing a failure. Skip it, and rely on the digits
 *    that ARE in view, which are measured for real.
 *
 ...
 * uses — nothing here is transient enough to need narrowing. The parameter is
...
export async function auditContrast(page: Page, within = 'body *'): Promise<ContrastFailure[]> {
  return page.evaluate((rootSelector) => {
     ...
     *
     * A driven scan here walks five `.mono-box` values, each a 617-digit
     * integer, the whole "what is a VDF" prose, a five-row comparison table,
     * the applications and constructions lists and the trapdoor panel, and
     * every one of those nodes re-walks the same handful of ancestors up to
     * `<body>`. Without the caches the pass re-reads the same computed styles
     * and rects thousands of times, and the gate scans eighteen states per
     * configuration. Nothing mutates the DOM during the pass, so the cached
     ...
     * that is the `.mono-box` value blocks and the `.progress` bar track.
     */
    ...
    const isVisible = (el: Element): boolean => {
      ...
      // paints nothing. This lab has exactly that case: the trapdoor panel is a
      // closed <details>. `checkVisibility()` catches it, and the gate opens it
      // by clicking its <summary> — the route a reader has — rather than
      // setting `.open` from script, which is what the previous gate did to
      // every <details> and hidden panel at once.
     ...
     * here may hide a real value behind that attribute.
     */
